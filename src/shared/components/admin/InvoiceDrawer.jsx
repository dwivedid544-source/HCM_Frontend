    ...
    setTimeout(() => {
      try {
            ...
            <div class="invoice-card">
              ...
              <table class="items-table">
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="amount">${formatCurrency(4000, 'USD')}</td>
                  </tr>
                  <tr>
                    <td>Extra Add-ons</td>
                    <td class="amount">${formatCurrency(280, 'USD')}</td>
                  </tr>
                  <tr>
                    <td>Tax (0% B2B)</td>
                    <td class="amount">${formatCurrency(0, 'USD')}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <div class="total-section">
                <div class="total-box">
                  <div class="total-row">
                    <span>Subtotal</span>
                    <span>${formatCurrency(4280, 'USD')}</span>
                  </div>
                  <div class="total-row">
                    <span>Tax (0%)</span>
                  ...
                  <div class="total-row grand-total">
                    ...
                    <span>${formatCurrency(cleanAmount(invoice.amount), 'USD')}</span>
                  </div>
                </div>
              </div>
        ...
        const file = new Blob([
          ...
          `Total Amount: ${formatCurrency(cleanAmount(invoice.amount), 'USD')}\n\n` +
          `Line Items:\n` +
          `- Enterprise Plan Subscription: ${formatCurrency(4000, 'USD')}\n` +
          `- Extra Add-ons: ${formatCurrency(280, 'USD')}\n` +
          `- Tax (0% B2B): ${formatCurrency(0, 'USD')}\n` +
          `=====================\n` +
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
               <div className="flex justify-between items-start">
                 ...
                 <div className="text-right">
                    <p className="text-[10px] font-bold font-bold text-slate-400 mb-1">Total Amount</p>
                    <p className="text-3xl font-black text-slate-900 tracking-tight">{formatCurrency(cleanAmount(invoice.amount), 'USD')}</p>
                 </div>
               </div>
 
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 font-bold">Invoice Date</p>
               ...
               <div className="space-y-4">
                  ...
                  <div className="space-y-3 font-bold text-slate-600 text-xs">
                     <div className="flex justify-between p-3 bg-white border border-slate-100 rounded-lg gap-4">
                        ...
                        <span className="shrink-0">{formatCurrency(4000, 'USD')}</span>
                     </div>
                     <div className="flex justify-between p-3 bg-white border border-slate-100 rounded-lg gap-4">
                        <span className="truncate">Extra Add-ons</span>
                        <span className="shrink-0">{formatCurrency(280, 'USD')}</span>
                     </div>
                     <div className="flex justify-between p-3 bg-white border border-slate-100 rounded-lg gap-4">
                        <span className="truncate">Tax (0% B2B)</span>
                        <span className="shrink-0">{formatCurrency(0, 'USD')}</span>
                     </div>
                  </div>
               </div>
