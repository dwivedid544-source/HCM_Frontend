          ...
          >
            <div className="p-6 sm:p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <div className="flex gap-2">
                 <button onClick={() => { navigator.clipboard.writeText(`LOG-${log.id}`); showToast('Copied Log ID'); }} className="p-2 hover:bg-slate-100 rounded-xl transition-all text-slate-400 shrink-0" title="Copy Log ID"><Copy size={20} /></button>
                 <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-xl transition-all text-slate-400 shrink-0"><X size={24} /></button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                     <span className="text-[10px] font-bold font-bold text-slate-400 block mb-1">Module</span>
                     <span className="text-sm font-bold text-slate-900">{log.module}</span>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                     <span className="text-[10px] font-black font-bold text-slate-400 block mb-1">IP Address</span>
...
{JSON.stringify({
   ...
   eventData: {
      ...
      status: 'success'
            ...
            <div className="p-6 sm:p-8 border-t border-slate-100 bg-slate-50 flex flex-col sm:flex-row gap-3 sm:gap-4 mt-auto shrink-0">
               <button onClick={() => showToast('Flagged event for security review', 'error')} className="flex-1 py-3.5 bg-rose-50 border border-rose-200 text-rose-600 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-rose-100 transition-all">
                  <AlertTriangle size={18} /> Flag Incident
               </button>
               <button onClick={() => showToast('Log details downloaded')} className="flex-1 py-3.5 bg-slate-900 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-black transition-all">
