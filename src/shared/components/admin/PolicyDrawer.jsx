...
const PolicyDrawer = ({ isOpen, onClose, policy, onEdit }) => {
  const { totalActiveEmployees } = useAdmin();
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl">
                  <div>
                      <p className="text-[10px] font-bold text-slate-400 font-bold">Effective Date</p>
                      <p className="text-sm font-bold text-slate-900 mt-1">{policy.effectiveDate || policy.date || 'TBD'}</p>
                  </div>
                  <div>
                      <p className="text-[10px] font-bold text-slate-400 font-bold">Status</p>
                      <p className={cn(
                  ...
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-4">
                     <div className="flex justify-between items-end">
                        <div>
                            ...
                            <span className="text-sm font-bold text-slate-400"> / {policy.acknowledgments ? policy.acknowledgments.split('/')[1] : totalActiveEmployees}</span>
                        </div>
                     ...
                     <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                        <div 
                           ...
                           style={{ width: `${(parseInt(policy.acknowledgments ? policy.acknowledgments.split('/')[0] : 0) / parseInt(policy.acknowledgments ? policy.acknowledgments.split('/')[1] : totalActiveEmployees)) * 100}%` }} 
                        />
                     </div>
                  </div>
               </div>
